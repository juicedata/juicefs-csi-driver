...
function main() {
  ...
  while true; do
    ...
    if [ $deployMode == "csi-on-demand" ]; then
      ...
      if [ $count = 1 ] && [ $all_count = 1 ]; then
        controller_pod=$(sudo microk8s.kubectl -n default get pods | grep Running | grep juicefs-csi-controller | awk '{print $1}' | cut -d/ -f2)
        sudo microk8s.kubectl cp default/$controller_pod:/usr/local/bin/juicefs /usr/local/bin/juicefs -c juicefs-plugin && \
            sudo chmod a+x /usr/local/bin/juicefs && juicefs -V
        sudo microk8s.kubectl cp default/$controller_pod:/usr/bin/juicefs /usr/bin/juicefs -c juicefs-plugin && \
