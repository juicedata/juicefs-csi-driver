...
RUN apt-get update && apt-get install -y librados2 curl fuse procps iputils-ping strace iproute2 net-tools tcpdump lsof librados-dev libcephfs-dev librbd-dev openssh-server openssh-client lrzsz && \
    rm -rf /var/cache/apt/* && \
