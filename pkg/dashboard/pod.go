...
func (api *API) upgradePods() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if nodeName == "" {
			c.String(400, "nodeName is required")
			return
		}
		podLog.Info("upgrade mount pod by batch", "node", nodeName, "recreate", recreate)

		csiNode, err := api.getCSINode(c, nodeName)
		if err != nil {
			podLog.Error(err, "get csi node error", "node", nodeName)
			c.String(500, "get csi node error %v", err)
			return
		}
		websocket.Handler(func(ws *websocket.Conn) {
			...
			podLog.Info("Start to upgrade mount pod", "node", nodeName, "recreate", recreate)
