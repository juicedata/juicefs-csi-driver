...
JUICEFS_EE_LATEST_VERSION=$(shell curl -sSL https://juicefs.com/static/juicefs -o juicefs && chmod +x juicefs && ./juicefs version | cut -d' ' -f3)
...
csi-slim-image-version:
	[ -z `git status --porcelain` ] || (git --no-pager diff && exit 255)
	docker buildx build -f docker/csi.Dockerfile -t $(IMAGE):$(VERSION)-slim --build-arg JUICEFS_REPO_REF=$(JUICEFS_LATEST_VERSION) \
