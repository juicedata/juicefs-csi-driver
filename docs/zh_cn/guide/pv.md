...
需要指出的是，调整 `subdir` 挂载参数，是更为推荐的方式。`subPath` 方式灵活性较差，在缓存预热场景中，或者子目录存在权限限制，`subPath` 方式均会遭遇报错，此参数多用于 CSI 内部调试，请勿用于生产环境。

### 动态配置

严格来说，由于动态配置本身的性质，并不支持挂载 JuiceFS 中已经存在的目录。但动态配置支持[「配置更加易读的 PV 目录名称」](#using-path-pattern)，你可以调整命名模板，让生成的子目录名称对齐 JuiceFS 中已有的目录，来达到同样的效果。
