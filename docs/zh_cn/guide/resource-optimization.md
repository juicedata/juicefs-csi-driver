...
JuiceFS Mount Pod 的 `requests` 默认为 1 CPU 和 1GiB Memory，`limits` 默认为 5 CPU 和 5GiB Memory。考虑到 JuiceFS 的使用场景多种多样，1C1G 的资源请求可能不一定适合你的集群，调整的时候需要注意：

* **不论 `requests` 如何调节，`limits.cpu` 都不建议低于 4**，过低的 CPU 限制会系统性降低 JuiceFS 挂载点的性能，即便在监控中发现实际资源用量很低，也应该保留比较宽容的 `limits.cpu`，默认为 5；
* `requests` 应该根据实际资源用量进行调整，比如通过监控发现 Mount Pod 只使用了 0.1 CPU、100MiB Memory，应该尊重实际监控数据，将 requests 调整为 0.1 CPU，100MiB Memory，避免过大的 `requests` 造成资源闲置，甚至导致容器无法调度，或者抢占其他应用容器（Preemption）；
* 通常情况下，不建议 `requests` 和 `limits` 差距过大。如果实际用量高于 `requests`，这便是「资源超售」（Overcommitment），严重的超售会影响集群稳定性。比方说 Mount Pod 的 `requests` 是 1 CPU、1GiB 内存，`limits` 是 16 CPU、64GiB 内存，实际资源用量达到 8 CPU、20 GiB 内存，此时虽然根据 `requests` 的定义，允许容器调度到节点上，但实际资源占用远高于 `requests`，节点可能出现各种资源挤占的问题，比如 CPU Throttle、OOM。因此这种情况下，应该根据实际用量，将 `requests` 调整为实际用量。
