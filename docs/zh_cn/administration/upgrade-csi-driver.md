...
* 如果已经通过[其他手段](../administration/upgrade-juicefs-client.md#update-mount-image)显式指定了 Mount 镜像，那么升级 CSI 驱动就不再“顺便”升级 JuiceFS 客户端版本了
...
如果 `k8s-online.yaml` 中[覆盖了默认的 Mount Pod 镜像](../administration/upgrade-juicefs-client.md#update-mount-image-csi-env)（可以通过 `JUICEFS_EE_MOUNT_IMAGE` 环境变量，或者 StorageClass 的 `juicefs/mount-image` 字段），并且指定了一个更老版本的 Mount Pod 镜像，我们鼓励你丢弃该配置，让集群随着 CSI 驱动升级，启用新版 Mount Pod 镜像，相当于伴随着 CSI 驱动升级，JuiceFS 客户端一并升级。
