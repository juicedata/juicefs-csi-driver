...
其中：
- `name`：JuiceFS 文件系统名称
- `token`：JuiceFS 管理 token。更多信息参考[这篇文档](https://juicefs.com/docs/cloud/metadata#token-management)
- `accesskey`：对象存储的 Access key。
- `secretkey`：对象出错的 Secret key。

您应该确保： accesskey 和 secretkey 对需要有对象存储 bucket 的 GET、PUT、DELETE 权限。
